...
int job(void *ptr) {
  ...
  curr_time = (ts.tv_sec - ref_ts.tv_sec) * 1000000l + (ts.tv_nsec - ref_ts.tv_nsec) / 1000l + period_nsec / 1000l;
  qmgr_periodic_get_deadline(&ps, &dl_ts);
  unsigned long delta_time = (ts.tv_sec - dl_ts.tv_sec) * 1000000l + (ts.tv_nsec - dl_ts.tv_nsec) / 1000l + period_nsec / 1000l;
  //printf("%d: %lu - %lu = %ld (abs_dt=%ld)\n", job_cnt, curr_time, ideal_time, delta_time, curr_time - first_time);
  ...
  if (num_elems > 0 && (job_cnt % queue_size/* (1000000000 / period_nsec) */) == 0) {
    printf("time=%ld, avg delay ever=%ld, max delay ever=%ld, avg delay=%ld, max delay=%ld\n",
	   curr_time, sum_delta_time / job_cnt, max_delta_time, enqueued_sum_dt / num_elems, max_qdt);
    fflush(stdout);
  }
