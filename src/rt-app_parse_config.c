...
static inline void assure_type_is(struct json_object *obj,
				  ...
				  const char *key, enum json_type type)
{
...
static inline char *get_string_value_from(struct json_object *where,
					  const char *key,
					  int have_def, const char *def_value)
{
	struct json_object *value;
	char *s_value;
...
static void parse_resources(struct json_object *resources,
...
static void serialize_acl(rtapp_resource_access_list_t **acl, int idx,
			  ...
			  rtapp_resource_t *resources)
{
	...
	/* as keys are string, we need a string for searching the resource */
	...
	if (!(*acl)) {
		*acl = malloc(sizeof(rtapp_resource_access_list_t));
		...
		if (found == 0) {
			/*
			 * add the resource to the acl only if it is
...
static void parse_thread_resources(const rtapp_options_t *opts,
				   ...
				   thread_data_t *data)
{
	...
	for (i = 0; i < json_object_array_length(locks); i++) {
		...
		debug_msg[0] = '\0';
		do {
			snprintf(tmpmsg, 512, "%s %d", debug_msg,
				 tmp->res->index);
		...
		if (!res) {
			...
			assure_type_is(res, task_resources, res_name,
					json_type_object);
			usage_usec = get_int_value_from(res, "duration",
							TRUE, 0);
...
static void parse_thread_data(char *name, struct json_object *obj, int idx,
			      thread_data_t *data, const rtapp_options_t *opts)
{
	...
	if (locks) {
		...
		log_info(PIN "key: lock_order %s",
			 json_object_to_json_string(locks));
		if (resources) {
...
static void parse_tasks(struct json_object *tasks, rtapp_options_t *opts)
{
	...
	foreach(tasks, entry, key, val, idx) {
		parse_thread_data(key, val, idx, &opts->threads_data[idx],
				  opts);
	}
}

static void parse_global(struct json_object *global, rtapp_options_t *opts)
{
	char *policy;
...
static void get_opts_from_json_object(struct json_object *root,
				      rtapp_options_t *opts)
{
...
/*
 * Read from stdin until EOF, write to temp file and parse as a
 * "normal" config file.
 */
