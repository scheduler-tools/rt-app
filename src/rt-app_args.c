...
void usage(const char *msg, int ex_code)
{
	...
	printf("-r, --rr\t\t:"
	       "\tset default policy for threads to SCHED_RR\n");
	...
	printf("-P, --pi_enabled\t:"
	       "\tenable priority inheritance on resources\n");

	printf("-M, --die_on_dmiss\t:"
	       "\texit with an error if a task misses a deadline\n");
